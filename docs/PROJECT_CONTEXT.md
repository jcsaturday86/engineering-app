...
The application form mirrors NBC Form No. B-03: Applicant Info + enterprise/ownership, cascading Applicant Address, Location of Construction (lot/block/TCT/tax-dec no.), a single-choice Scope of Work (`new_construction`/`erection`/`addition`/`repair`/`others`, with a detail textbox), a fixed Design Professional block (name/address/PRC no./validity/PTR no./date issued/issued at/TIN), an identically-shaped fixed Full-Time Inspector or Supervisor block, and Consent of Lot Owner (name/address/CTC no./date issued/issued at). Like DP/SGP, there's no `printForm()` on the application itself — only the final permit certificate prints.
