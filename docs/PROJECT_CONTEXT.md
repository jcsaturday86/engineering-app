...
### Permits List Redesign (`/permits/building`, `/permits/occupancy`, `/permits/demolition`, `/permits/signage`)
`permits/index.blade.php` (shared by all 4 permit types, keyed by a `$type` variable) gained:
