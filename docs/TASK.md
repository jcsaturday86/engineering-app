...
### Elevator/Escalator/Machinery Annual Inspection Certificates & Verify-Page Fix — COMPLETED

- Built three new background-image-overlay AI certificate templates matching the GE certificate's real-scanned-form technique, replacing the generic `pdf/annual-inspection-permit.blade.php` for their groups: `annual-inspection-permit-elevator.blade.php` (ELEV, per-unit), `annual-inspection-permit-escalator.blade.php` (ESC, per-unit), `annual-inspection-permit-machinery.blade.php` (MACH, bundled — all non-ACREF/ELEV/ESC `AINSP_MECH` items on one cert)
- Each wired into `PermitController::print()` via a new `$isAiElev`/`$isAiEsc`/`$isAiMach` flag and a scoped `dpi=128` DomPDF option — matching each background's own native resolution (measured `imagesx()/imagesy() ÷ page-size-in-inches`) rather than GE's `dpi=200`, since these backgrounds are smaller-source scans that would just be upscaled for no visible gain
- All three backgrounds converted from the user-supplied PNGs to flattened, GD-unsharp-masked JPEGs (`imageconvolution` sharpen kernel before `imagejpeg(..., 92)`) — same PNG-embedding-perf fix already applied to every other background-overlay form in this app, plus a new sharpening step for perceived crispness at no extra render cost; original PNGs kept on disk unreferenced as calibration sources
- Elevator/Escalator: right page gained a centered city+province letterhead line and an `ai_city_engineer` name+designation block above the QR code (Escalator's physical form has no such block, so it was left off there); left page gained the `ai_mechanical` signatory name above "Chief, Mechanical Inspection" (previously left blank)
- Machinery's Description field previously got clipped mid-word for long text; fixed with `wordwrap()` splitting onto the form's second printed line, whose true left edge (measured separately from line 1, since line 1 is indented under its label but line 2 is a full-width blank rule) required its own calibration
- Machinery's QR code went through several size/position rounds (bigger, moved right, moved down) constrained by a tight ~0.75–0.8in physical gap between the "Date Paid" line and the pre-printed "NOTE:" text (measured via GD pixel-scan after an initial `+25%` request proved infeasible and was explained to the user rather than silently under-delivered); settled at a ~10% size increase from the original placement
- Fixed a real bug found while reviewing a public verify link for an AI permit: `AnnualInspectionApplication` stores a single `owner_name`/`location_street` instead of the split `applicant_*_name`/`building_street` fields every other application type uses, so `/verify/permit/{token}` showed a blank Applicant and Location for AI permits. `VerifyController::show()` now duck-types on `owner_name` presence; `verify/permit.blade.php`'s Location row falls back to `location_street`

### Air-Conditioning/Refrigeration (ACREF) Certificate & PDF Favicon Links — COMPLETED

- Built the fourth and final dedicated AI certificate template, `annual-inspection-permit-acref.blade.php` — the backend grouping logic (`AI_ACREF_CODES`, `buildAiCertificateGroups()`'s `ACREF` bundle, `AnnualInspectionEquipmentItem::acRefCodes()`) already existed from an earlier session but had no print branch; added `$isAiAcref`, the `pdf.annual-inspection-permit-acref` template mapping, and the `dpi=128` branch alongside Elevator/Escalator/Machinery
- Reproduces NBC Form No. M.10: left page "Certificate of Operation" (owner/address, "Verified as to the following" equipment list, fee/date fields, a "Mechanical Permit No./Issued On" line reusing the underlying AI permit number, and 4 signature blocks — `ai_mechanical`, `ai_chief_inspection_enforcement`, `ai_chief_processing_evaluation`, plus "Mechanical Inspector" left blank since no signatory role exists for it); right page "Office of the Building Official" operation certificate with an `ai_city_engineer` name+designation block and QR code, all field positions located via the session's established GD pixel-scan calibration technique
- Added, at follow-up request, an `ai_city_engineer` designation line beneath the name (initially placed in a too-tight gap that collided with the pre-printed "(Signature over Printed Name)" caption; moved above the signature line instead, in the same blank space already used for the name)
