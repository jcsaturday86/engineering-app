...
**Key settings:** `assessment.electrical_inspection_percentage` (default 10), `assessment.default_filing_fee`, `assessment.default_processing_fee`, `general.logo` (type `file` — city/LGU official seal, uploaded via Settings → General, GD-resized to max 400px before storage; printed on permits, application forms, assessment summaries, billing statements, official receipts, and used as the browser-tab favicon fallback), `general.favicon` (type `file` — dedicated browser-tab icon shown on every page; falls back to `general.logo`, then to the static `favicon.ico`), `general.dpwh_logo` (type `file` — DPWH logo for the Occupancy Permit PDF and the Building Permit PDF's right header cell; falls back to the static `public/images/dpwh-logo.png` asset when empty), `general.national_govt_logo` (type `file` — National Government logo shown on the right of the BP Unified Application Form letterhead and the left of the OP Application Form header), `general.city` / `general.province` (printed on PDF letterheads; seeded as "City of San Fernando" / "La Union"), `general.area_number` (Area No. digit box on the BP application form when the application has none of its own), `general.zip_code` (Building Permit PDF), `general.domain` (public domain used to build the QR verification link on printed permits; blank falls back to `config('app.url')`), `general.api_key` (type `api_key` — 64-char random hex credential for a future external API integration; displayed/regenerated via a dedicated "API Access" card on `/settings`, kept outside the main Settings form so a routine save can never overwrite it; treated like `file` in the seeder's reseed guard — `firstOrCreate`, never reset by a reseed).
...
- `signatories` — id, role, name, title, designation, department, license_no, is_active. The `building_official` role is used as "Approved By" on the assessment summary PDF and permits. Also seeded with 15 `ai_*` roles (`ai_locational_zoning`, `ai_line_and_grade`, `ai_architectural`, `ai_civil_structural`, `ai_electrical`, `ai_mechanical`, `ai_sanitary`, `ai_plumbing`, `ai_electronics`, `ai_interior_design`, `ai_accessibility`, `ai_fire_safety`, `ai_chief_inspection_enforcement`, `ai_chief_processing_evaluation`, `ai_city_engineer`) for the Annual Inspection "General, Occupancy & Electrical" certificate's discipline signature blocks (see Workflows). Settings > Signatories only supports **editing** existing rows (no create/delete route) — new roles are seeded via `ReferenceDataSeeder`, not entered through the UI.
...
| **Processing** | entered_by, assessed_by, approved_by, client_user_id (all FK → users), submitted/assessed/approved/paid/released/cancelled_at, cancellation_reason, issued_date |
| **Occupancy reference** | occupancy_no (string, nullable), occupancy_issued_date (date, nullable) — both optional; feed the GE certificate's pre-printed "Certificate of Occupancy No. ___ issued on ___" line, previously left permanently blank for lack of a data source |
| **System** | deleted_at |

**Indexes:** [status], [app_year, app_month]

> **History note:** this table (and its model/controller) originally shipped as `mechanical_applications`/`MechanicalApplication`/`MechanicalApplicationController`, backing a 5-equipment-tab "Mechanical Permit" (MP) module with multi-permit generation. A later rename migration renamed the table, backfilled `PermitType.code` (`MP`→`AI`), and every route/morph/model reference to `AnnualInspectionApplication`/`AI`, once the module's fee tabs were rebuilt around the official Annual Inspection Fees schedule. The form started out deliberately minimal (owner name + location only) and later gained an Equipment/Items checklist (see `annual_inspection_equipment_items` below), a Character of Occupancy field (single-select radio, via `application_occupancy_groups` — see above), and the optional `occupancy_no`/`occupancy_issued_date` pair above — actual assessed fee/quantity data still lives in the Assessment, not the application record.

### `annual_inspection_permit_units`
`mechanical_application_id` (FK, cascade — column name retained from the pre-rename schema), `assessment_item_id` (FK → assessment_items, nullable — set only for per-unit Elevator/Escalator certificates, so print-time lookup never has to re-derive "which unit is this"), `group_code` (`GE`, `ELN`, `MACH`, `ACREF`, `ELEV`, or `ESC`), `description`, `quantity`, `amount`, `permit_id` (FK permits, nullable), `generated_at`, `deleted_at`. Originally built for the module's first multi-permit-per-application design, then left dormant when Permit Generation was switched to single-permit-per-application — since **reactivated**: AI generation now produces up to 6 certificates per application (one row per certificate here, one `Permit` each), grouped by inspection discipline rather than the original equipment-tab categories. Bundle-type certificates (`GE`/`ELN`/`MACH`/`ACREF`) leave `assessment_item_id` null; per-unit certificates (`ELEV`/`ESC`) set it to the specific elevator/escalator `AssessmentItem`. Every `group_code` except `ELN` now has its own background-image-overlay print template (`pdf/annual-inspection-permit-{ge,elevator,escalator,machinery,acref}.blade.php`, selected by `PermitController::print()`'s `$isAi*` flags — see `docs/WORKFLOWS.md`); `ELN` still falls back to the generic `pdf/annual-inspection-permit.blade.php`.
...
### `billings`
Polymorphic, `billing_number` (BL-YYYY-MM-NNNNN), `collection_reference_no` (string(20), nullable, unique), `total_amount`, `status` (unpaid/partial/paid/void), `generated_by`.
...
>
> `collection_reference_no` is generated the same way, in the same transaction: `40000` (fixed system code) + `YYMM` + a 4-digit sequence that resets every month, e.g. `4000026080001`. Because `billings` is shared across all 6 permit types, this is one monthly counter, not six — a BP and an OP finalized back-to-back in the same month get consecutive numbers. Existing billed/paid rows were backfilled via `php artisan billings:backfill-reference-no` (chronological, bucketed by each row's own `created_at` month). Used as the barcode payload on the printed Summary of Computation (all 6 types) and as a searchable field on `/collections`, in preparation for an external API integration.
