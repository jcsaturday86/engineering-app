...
| annual-inspection-permit | PermitController::print (AI, group ELN only — the sole remaining group on the generic template) — one certificate per print, parameterized by the specific `AnnualInspectionPermitUnit`; itemized fee table, QR verification code |
| annual-inspection-permit-ge | PermitController::print (AI, group GE only) — background-image-overlay reproduction of NBC Form No. B-19, single A4-landscape page, auto-filled from application/assessment/permit + the 15 `ai_*` Signatory roles (locked into `permits.signatories_snapshot` at generation time), plus a QR verification code |
| annual-inspection-permit-elevator | PermitController::print (AI, group ELEV, per-unit — one cert per elevator item) — background-image-overlay of the Elevator Annual Inspection certificate (`elevator.jpg`); left page = Certificate of Inspection, right page = Office of the Building Official's Certificate of Operation with city/province letterhead and `ai_city_engineer` name+designation block; `ai_mechanical` signatory above "Chief, Mechanical Inspection"; QR code |
| annual-inspection-permit-escalator | PermitController::print (AI, group ESC, per-unit — one cert per escalator/funicular/cable-car item) — same two-page layout/conventions as Elevator (`escalator.jpg`), 8 spec fields (rated load, capacity/hr, speed, effective width, tread width, floors served, floor height, motor HP) on both pages; no `ai_city_engineer` block (not present on this physical form) |
| annual-inspection-permit-machinery | PermitController::print (AI, group MACH, bundle — all non-ACREF/ELEV/ESC `AINSP_MECH` items on one cert) — background-image overlay (`machinery.jpg`); Description field word-wraps onto a second printed line when it overflows; right page lists up to 4 item lines; `ai_mechanical` + `ai_city_engineer` signatories; QR code |
| annual-inspection-permit-acref | PermitController::print (AI, group ACREF, bundle — all `AINSP_FI_REFRIG`/`AINSP_FII_WINAC`/`AINSP_FIII_CENAC` items on one cert) — background-image overlay (`aircon.jpg`) of NBC Form No. M.10; "Verified as to the following" bulleted equipment list (`• N Unit(s) of {label}: {description} — {tons_or_hp}`, `quantity_count` from `computation_details`, not the `quantity` column which holds the ton/HP input value) rendered on **both** pages via a `@foreach` loop sized to fit up to 10 bullets in the available blank space; 4 left-page signature blocks (`ai_mechanical`, `ai_chief_inspection_enforcement`, `ai_chief_processing_evaluation` + one left blank — no signatory role exists for "Mechanical Inspector"); `ai_city_engineer` name+designation above the QR on the right page |
...
- **Found** → `verify/permit.blade.php` shows permit type (Building Permit / Certificate of Occupancy / Demolition Permit / Signage Permit / Fencing Permit / Annual Inspection Permit — a `match()` on `permitType->code`; FP and AI were both missing until a bug fix, silently falling into the `default` "Building Permit" label), permit number, date issued, **Issued By** (the snapshotted Building Official — title/name/designation, immutable per permit, see the Building Official Snapshot note in `docs/PROJECT_CONTEXT.md`), status, applicant name, project title, location
