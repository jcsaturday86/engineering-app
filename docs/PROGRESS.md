...
| Print Forms dropdown | DONE | BP Show page: single "Print Forms" dropdown (Alpine) replacing 7 separate buttons — Application Form + 6 discipline forms (Architectural/Structural/Electrical/Sanitary/Mechanical/Electronics); the online client portal's BP show page now renders the identical dropdown (was a single "Print Application Form" link), via new `online.print.discipline` route + `OnlineApplicationController::printDiscipline()` |
| Client print availability | DONE | Print is available at any application status, including `draft` — for all 6 permit types. Removed a prior `approved_at !== null` gate from both the Blade buttons and `OnlineApplicationController::printForm()`/`printDiscipline()`; a client can now print/preview a form before submitting it |
| Skip Locational Clearance hidden for online clients | DONE | The checkbox on `applications/form.blade.php` is gated `@if($isBP && $portal !== 'client')` — staff (including staff editing an online-originated application) retain it; online clients (`/online/apply?type=BP`) never see it |
...
| Staff "Print" button on Show page | DONE (fixed) | Route/controller method existed but no UI button rendered it — staff could not reach it except by typing the URL; added the same single "Print" link used by OP/FP/DP |
...
| Verify-permit page: AI applicant name/location bug fix | DONE | `AnnualInspectionApplication` doesn't have the split `applicant_*_name`/`building_street` fields used elsewhere — it has `owner_name`/`location_street` instead, so both the Applicant and Location rows showed blank for AI permits. `VerifyController::show()` duck-types on `owner_name` presence; the blade falls back to `location_street` |
| Elevator Annual Inspection certificate | DONE | `pdf/annual-inspection-permit-elevator.blade.php` — background-image overlay (own scan, `elevator.jpg`), one certificate per elevator item (`group_code = ELEV`, per-unit); left page Certificate of Inspection, right page Office of the Building Official Certificate of Operation with centered city/province letterhead and an `ai_city_engineer` name+designation block; `ai_mechanical` signatory above "Chief, Mechanical Inspection"; QR code |
| Escalator Annual Inspection certificate | DONE | `pdf/annual-inspection-permit-escalator.blade.php` — same layout/conventions as Elevator (`escalator.jpg`), one cert per escalator/funicular/cable-car item (`group_code = ESC`); 8 spec fields (rated load, capacity/hr, speed, effective width, tread width, floors served, floor height, motor HP); no `ai_city_engineer` block (this physical form has no such signature line) |
| Machinery Annual Inspection certificate | DONE | `pdf/annual-inspection-permit-machinery.blade.php` — bundled certificate (all non-ACREF/ELEV/ESC `AINSP_MECH` items on one cert, `group_code = MACH`, not per-unit); Description field word-wraps onto a second printed line when it overflows the first (`wordwrap()`), right page lists up to 4 item lines; `ai_mechanical` + `ai_city_engineer` signatories; QR code |
| Air-Conditioning/Refrigeration (ACREF) Annual Inspection certificate | DONE | `pdf/annual-inspection-permit-acref.blade.php` — bundled certificate (all `AINSP_FI_REFRIG`/`AINSP_FII_WINAC`/`AINSP_FIII_CENAC` items, `group_code = ACREF`) reproducing NBC Form No. M.10; "Verified as to the following" bulleted equipment list (`• N Unit(s) of {label}: {description} — {tons_or_hp}`) on both pages, spaced to fit up to 10 items; 4 left-page signature blocks (`ai_mechanical`, `ai_chief_inspection_enforcement`, `ai_chief_processing_evaluation` + "Mechanical Inspector" left blank — no signatory role exists for it); `ai_city_engineer` name+designation above the QR code on the right page |
| AI certificate background performance/quality | DONE | Elevator/Escalator/Machinery/ACREF backgrounds are flattened, unsharp-masked JPEGs (GD convolution sharpen before `imagejpeg(..., 92)`) rendered at `dpi=128` — each background's own native resolution — rather than GE's `dpi=200` (which would upscale these smaller-source images for no visible gain); all four render in ~1s |
| PDF template favicon links | DONE | All 34 `resources/views/pdf/*.blade.php` templates gained a `<link rel="icon">` tag for source consistency; ACREF's resolves from `general.favicon` (mirrors the app's own favicon logic) instead of the static asset. Has no visible effect on the rendered PDF — DomPDF output is binary, and browsers' PDF viewers don't read HTML `<head>` tags |
| Staff "Print" button on Show page | DONE (fixed) | Same gap as SGP — `annual-inspection-applications.print` route/controller existed with no UI button; added |
...
| Document requirement management | DONE | `Settings → Document Requirements`, new `manage-document-requirements` permission (auto-granted to super-admin/administrator via the seeder's `reject()` pattern). Drill-down index (card per service, total + mandatory counts) → per-service editor with inline row edit, add-row, nest-under-parent, activate/deactivate, delete. Drives what online clients must upload — see the Online Client Portal section |
...
| Requirement-document upload gate before submission | DONE | Now a **mandatory-requirement** gate, not a bare count: `submit()` calls `ClientWizard::missingMandatory()` and refuses with the missing documents named. A service with no mandatory rows configured (currently AI) passes automatically — no per-type code. Upload action available while `draft`/`returned` |
| **Configurable document requirements per service** | DONE | New `Settings → Document Requirements` (`manage-document-requirements`): new `document_requirements` table + `DocumentRequirement` model + `DocumentRequirementController` + drill-down index/per-type editor views. Two-level structure (headings + nested items), three obligation levels (mandatory/conditional/optional), `condition_note`, `is_uploadable`, `is_active`, `sort_order`. Seeded from the official Engineering Office lists — BP 30, FP 18, OP 6, DP 5, SGP 5, AI 0 (64 rows) |
| Client upload page as a checklist | DONE | Replaced the single form + generic 11-option dropdown with a per-row checklist (`partials/requirements-checklist.blade.php`): level badges, condition notes, per-row Upload/View/Replace/Delete, and an "N of M mandatory uploaded" progress bar. Replace deletes the superseded file first; Delete removes row **and** file, so it genuinely frees disk space |
| Client creation wizard (Details → Documents → Submit) | DONE | New `app/Support/ClientWizard.php` + `partials/client-wizard-stepper.blade.php`, shown on the 6 form views, the upload page, and the 6 detail pages while `draft`/`returned`. Step is derived from mandatory-document completion, never stored. Saving the form (create **or** edit) now redirects straight to the upload page instead of the detail page |
| Uploaded document viewing | DONE | New `ApplicationRequirementController` + `requirements.view`/`requirements.download` routes, authorized per-record (staff with `view-applications`, or the owning client). View/download links on the client checklist and — via the new shared `partials/requirements-uploaded-list.blade.php` — in the Requirements card on all 6 detail pages. Closed a real gap: reviewers previously had no way to open the document they were approving |
...
| Dashboard / Applications-list split | DONE | `online.dashboard` is now a lean landing page (welcome banner, 4 stat cards, Recent Applications ×5 with a "View All" link); the full filterable record list moved to a new `online.applications` route + view. Both share a private `getClientApplications()` helper. The sidebar's "All Applications" link and the "My Applications" breadcrumbs on apply/upload/track now point at the new route (previously both nav entries pointed at the dashboard) |
| Apply-page polish | DONE | "How it works" info icon enlarged to `text-2xl` and its step-number circles switched from blue to gray (`bg-gray-400`, rail `bg-gray-200`); matching `fa-list-check` icon added beside "Select a permit type" |
...
| Client application-form printing (all 6 types) | DONE | New `printForm()` + PDF templates for Signage and Annual Inspection (the two types with no prior application-form print) — all 6 types now have a client print route. **Later session**: the approval gate was removed (available at any status, including `draft`); BP's client print button became the same "Print Forms" dropdown (Application Form + 6 discipline forms) staff has, via new `online.print.discipline` route |
| Staff monitoring: Source (Online/Onsite) column + filter | DONE | All 6 staff application index tables gained a Source badge column and a `?source=` filter dropdown, so staff can distinguish online-submitted from walk-in applications in the monitoring lists |
