...
Same shape again, trimmed further (no enterprise/CTC/inspector/lot-owner sections, no occupancy-group selection). `printForm()` (route `signage-applications.print`, `pdf/signage-application-form.blade.php`) was added in a later session, once online self-service and its "unlock print after approval" gate made a print-less SGP application form a real gap, not just a deferred nice-to-have. The route/controller method existed for a while with **no button in the staff `show.blade.php` UI** — staff could only reach it by typing the URL; fixed in a later session by adding the same single "Print" link every other type's staff show page already has.
...
Same shape as `FencingApplicationController` — trimmed to AI's minimal field set (owner name, location, `application_kind` new/yearly toggle, editable only while `draft`). No occupancy-group selection, no enterprise/CTC/inspector/lot-owner sections. Renamed from `MechanicalApplicationController` in the same rename pass as the model/table. `printForm()` (route `annual-inspection-applications.print`, `pdf/annual-inspection-application-form.blade.php`) was added in the same later session as Signage's, for the same online-portal-driven reason — and had the same missing-staff-button gap, fixed alongside Signage's.

**Source filter, all 6 controllers**: each `index()`/`filteredQuery()` accepts `?source=` (`walk_in`/`online`) alongside the existing `search`/`status`/`year` filters — `if ($request->filled('source')) { $query->where('source', $request->source); }` — added identically across `ApplicationController`, `OccupancyApplicationController`, `FencingApplicationController`, `DemolitionApplicationController`, `SignageApplicationController`, `AnnualInspectionApplicationController` so staff monitoring can isolate online-submitted vs. walk-in applications; see `docs/WORKFLOWS.md`'s "Staff Monitoring: Online vs Onsite".
...
`report(Request $request)` now requires both `date_from` and `date_to` — returns `zoning/report-form.blade.php` (a date-range + status picker) whenever either is missing, instead of generating a PDF from an unbounded/implicit range. The PDF (shared `pdf/report.blade.php`) gained a "Total Zoning Assessment" column and a narrower `#` column.
...
- `doPrint(PermitApplicationContract)` — dispatches by `getPermitTypeCode()`: BP renders `pdf.assessment-summary` (building + zoning sections); OP/DP/SGP/FP/AI each delegate to their own `doPrintOp()`/`doPrintDp()`/`doPrintSgp()`/`doPrintFp()`/`doPrintAi()`, rendering `pdf.assessment-summary-{op,dp,sgp,fp,ai}` with only that permit type's single fee-category section (no Zoning/Building/Other Fees/Filing/Processing). All six generate a Code 128 barcode (picqer BarcodeGeneratorPNG, base64) and load the `building_official` signatory. The barcode payload is `$application->billings()->latest()->value('collection_reference_no')` (falling back to `application_number` only if no billing exists yet — shouldn't happen, since the Print button only shows once finalized, by which point billing generation has already run) — was `application_number` alone until the Collection Reference Number feature (see `docs/PROJECT_CONTEXT.md`).
...
- `index(Request $request)` — accepts `search`: exact match on a billed BP/OP/DP/SGP `application_number` **or** its unpaid billing's `collection_reference_no` redirects straight to that payment form (barcode-scan UX — the Summary of Computation barcode now encodes the reference number); partial match filters the Awaiting Payment list by application number, reference number, or applicant name. Also accepts `month` (`YYYY-MM`, defaults to current month) — the Payment History list is scoped to the logged-in collector's own transactions (`collected_by = Auth::id()`) within that month
...
`generate`/`generateOp`/`generateDp`/`generateSgp` (via `doGenerate()`) set a `verification_token` (UUID) on the new `Permit` row, and snapshot the currently-active `building_official` Signatory onto `building_official_name`/`_title`/`_designation`/`_license_no` — a one-time capture that survives Signatory edits, revoke, and restore. `print()` additionally builds a QR code (`endroid/qr-code`) encoding the public verification URL (`{general.domain setting|app.url}/verify/permit/{token}`) and passes it (plus `sealImage`, `dpwhLogo` — both sourced from `Setting`, each falling back to a static default) to the template selected by `match ($permit->permitType->code) { 'OP' => 'pdf.occupancy-permit', 'DP' => 'pdf.demolition-permit', 'SGP' => 'pdf.signage-permit', 'AI' => 'pdf.annual-inspection-permit', default => 'pdf.building-permit' }`, which reads the Building Official line from the permit's own snapshot columns, not the live Signatory. For AI specifically, `print()` looks up the `AnnualInspectionPermitUnit` tied to the `$permit->id` and passes it (plus, for bundle-type certificates, a re-derived itemized item list scoped to that unit's `group_code`) instead of the generic BP/OP/DP/SGP variable set; `$isAiGe`/`$isAiElev`/`$isAiEsc`/`$isAiMach`/`$isAiAcref` flags (each `$aiUnit->group_code === '<CODE>'`) override the template match to `pdf.annual-inspection-permit-{ge,elevator,escalator,machinery,acref}` respectively (only group `ELN` still falls through to the generic `pdf.annual-inspection-permit`). GE gets `$pdf->setOption('dpi', 200)`; the other four backgrounds (`elevator.jpg`/`escalator.jpg`/`machinery.jpg`/`aircon.jpg`) are matched at `dpi=128` — each background's *native* resolution (measured `imagesx()/imagesy() ÷ page-size-in-inches`), so DomPDF isn't upscaling to 200dpi for no visible gain. All five backgrounds are flattened, unsharp-masked JPEGs (`imageconvolution($img, [[-1,-1,-1],[-1,16,-1],[-1,-1,-1]], 8, 0)` on the truecolor source before `imagejpeg(..., 92)`) regenerated from the original PNGs, which are kept on disk unreferenced as calibration sources. `doGenerateAi()` additionally snapshots the 14 `ai_*` discipline/Chief Signatory rows into `signatories_snapshot` on each generated `Permit`, alongside the existing `building_official_*` snapshot fields — same locking principle, scoped to `doGenerateAi()` only since no other permit type's template reads these roles.

Every `resources/views/pdf/*.blade.php` template's `<head>` has a `<link rel="icon">` tag pointing at `asset('favicon.ico')` (added for source consistency across all 34 PDF views). It has no effect on the rendered PDF or on what icon shows when a browser opens it — DomPDF converts the Blade template to a binary PDF, and browsers display PDFs in a dedicated viewer that doesn't read HTML `<head>` tags at all. `annual-inspection-permit-acref.blade.php` alone resolves this dynamically from `$settings['general.favicon']` (mirroring `partials/favicon.blade.php`'s Setting-driven resolution, falling back to the same static `favicon.ico`) rather than hardcoding the asset path, per a follow-up request — same no-visible-effect caveat applies.
...
`SettingsController::storeUser()` validates and applies an admin-supplied password (`Password::min(8)->mixedCase()->numbers()->symbols()`, `confirmed`) instead of hardcoding every new staff account to `password123`. `update()` (Settings → General file uploads) derives each file setting's storage path from its key via a `match` expression, rather than a single hardcoded path shared by all file settings. `users()` accepts `?tab=staff|clients` (staff default) plus a search/role filter, replacing the old single mixed staff+client listing. New `regenerateApiKey()` rotates `general.api_key` (`bin2hex(random_bytes(32))`) and flashes a success message; `index()` also passes `$apiKeySetting` separately from the generic `$settings` collection so the API Access card can render outside the main Settings form.
...
| BillingService | generateFor() — auto-create billing on assessment finalize (all 6 permit types), set status to billed. Internal `$morphType` match must carry all 6 permit-code arms — was found missing the `SGP` arm during the SGP build (silently created billings with `applicationable_type = 'bp'`). Also generates `collection_reference_no` in the same transaction (`40000` + `YYMM` + 4-digit monthly-reset sequence, prefix+`LIKE`+suffix-increment, same pattern as `billing_number`) |
...
`bp:remove-inspection-fees` (`--dry-run`) — retroactive BP Electrical/Mechanical inspection-fee zeroing, see `docs/TASK.md`. `billings:backfill-reference-no` — one-off command assigning `collection_reference_no` to any existing `Billing` row that predates the column, in chronological order, bucketed by each row's own `created_at` month (mirrors the live-generation logic in `BillingService::generateFor()` so backfilled and newly-generated numbers stay in the same sequence).
...
`sidebar-nav.blade.php` computes three role-flag booleans at the top of its `@php` block — `$isPlanningOnly`/`$isTreasuryOnly`/`$isEngineeringOnly` (each `hasAnyRole([officer, staff]) && !hasAnyRole(['super-admin','administrator'])`) — used via `@unless(...)`/`@if(...)` to scope which menu sections render for those 3 roles. See `docs/PROJECT_CONTEXT.md`, "Role-Scoped UI (Planning / Treasury / Engineering)".
...
`collections/create.blade.php` — POS-style single-screen payment form: Application No./Applicant/Reference No. row (3-column), OR Number/Paid By row (2-column — kept as the only two fields in that row so they sit side by side), a 3-column Amount Due/Amount Received/Change strip (Alpine-live), a Cash/Check/Online segmented control, and a sticky bottom action bar so the collector doesn't scroll mid-transaction.

`zoning/report-form.blade.php` — new date-range + status picker shown by `ZoningController::report()` when the request has no `date_from`/`date_to`; mirrors `reports/permits.blade.php`'s styling, reuses `zoning/index.blade.php`'s `$statusLabels` for its Status dropdown.

`dashboard/planning.blade.php` / `dashboard/treasury.blade.php` — new role-scoped dashboards rendered by `DashboardController::planningDashboard()`/`treasuryDashboard()` instead of the normal KPI dashboard, for users whose only roles are `planning-officer`/`planning-staff` or `treasury-officer`/`treasury-staff` respectively (see `docs/PROJECT_CONTEXT.md`, "Role-Scoped UI"). Treasury's dashboard: today's/monthly/annual revenue + transaction-count stat cards, a year-navigable monthly revenue Chart.js bar chart, and a Recent Collections list linking to `collections.receipt` (built via `Collection::where('status','active')` — note the `applicationable` relation, not the `application` accessor, must be eager-loaded for the recent-collections list to resolve each row's application).
...
**`official-receipt.blade.php`**: header department line reads "Office of the City Treasurer" (was "Office of the Building Official — Engineering Department", incorrect — Treasury issues the receipt, not Engineering). Body font is `'DejaVu Sans', Arial, sans-serif` (was `Arial` alone, which DomPDF maps to core Helvetica — no ₱/U+20B1 glyph, so every peso amount rendered as a missing-glyph box; same fix as the Peso Sign Fix in `docs/TASK.md`, just not originally applied to this template). `OnlineApplicationController::doDownloadPermit()` (client-portal permit download) passes the same full variable set (`settings`/`sealImage`/`dpwhLogo`/`qrImage`) as `PermitController::print()` — it previously passed none of them, so downloaded permits silently rendered without seal/logo/QR.
