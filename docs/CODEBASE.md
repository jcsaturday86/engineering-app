...
`printForm()` streams a real DomPDF (`defaultMediaType=print`, `dpi=200`) instead of returning browser-print HTML. `printDiscipline(Application, string $discipline)` (`GET /applications/{id}/print-discipline/{discipline}`) validates against the `DISCIPLINE_FORMS` const (architectural/structural/electrical/sanitary/mechanical/electronics); `architectural`, `structural`, and `electrical` dispatch to `printArchitecturalForm()` / `printStructuralForm()` / `printElectricalForm()` (real NBC/permit-form PDFs, background-image overlay, own scans), the rest render a shared blank placeholder (`pdf/discipline-form.blade.php`). All three dedicated print methods share a private `resolveBuildingOfficial(Application): array` helper — returns `[title, name, designation]` from the first generated `Permit`'s snapshot, or the active `building_official` Signatory if no Permit exists yet.
...
**`discipline-form.blade.php` (DomPDF)** — shared blank placeholder for Sanitary/Mechanical/Electronics (no official source form digitized yet for these); A4 portrait, city seal header, form title from `ApplicationController::DISCIPLINE_FORMS`.
