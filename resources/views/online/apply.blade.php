...
<div class="max-w-6xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-baseline justify-between gap-1 sm:gap-4 mb-4">
        ...
        <a href="{{ route('online.dashboard') }}" class="text-sm text-gray-500 hover:text-gray-800 shrink-0">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,5fr)_minmax(0,6fr)] gap-6 items-stretch">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4 flex flex-col h-full">
            <h3 class="text-base font-semibold text-gray-900 mb-2">
                <i class="fas fa-circle-info text-blue-500 mr-1.5"></i> How it works
            </h3>

            <ol class="relative flex-1 flex flex-col justify-between">
                {{-- vertical rail joining the step numbers --}}
                <span class="absolute left-4 top-4 bottom-4 w-px bg-blue-100" aria-hidden="true"></span>

                @foreach($steps as $i => $step)
                <li class="relative flex items-center gap-4">
                    <span class="relative z-10 flex items-center justify-center w-8 h-8 rounded-full bg-blue-600 text-white text-sm font-semibold shrink-0 ring-4 ring-white">
                        {{ $i + 1 }}
                    </span>
        ...
        {{-- ============================================================== --}}
        <div class="bg-white rounded-xl border border-gray-200 p-4 flex flex-col h-full">
            <h3 class="text-base font-semibold text-gray-900 mb-2">Select a permit type</h3>
