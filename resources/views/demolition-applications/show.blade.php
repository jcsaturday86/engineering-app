...
@section('title', 'DP Application Details')
...
@php
    ...
    $sectionNum = 0;
    $portal = $portal ?? 'staff';
    $applicationType = $applicationType ?? 'DP';
...
<div class="space-y-4">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3">
                <div>
                    ...
                    <div class="flex flex-wrap items-center gap-2 mt-1">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium bg-red-100 text-red-700">
                            {{ $application->getPermitTypeCode() }} &mdash; Demolition Permit
                        </span>
            ...
            <div class="flex flex-wrap items-center gap-2" x-data="{ showRevertSubmitModal: false, revertSubmitPassword: '', showSubmitModal: false, submitPassword: '' }">
                @if($application->status === 'draft')
                    <a href="{{ route('demolition-applications.edit', $application) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 transition">
                    ...
                    <div x-show="showSubmitModal" x-cloak
                        ...
                        <div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6" @click.outside="showSubmitModal = false">
                            ...
                            <form method="POST" action="{{ route('demolition-applications.submit', $application) }}" autocomplete="off">
                                @csrf
                                <div class="mb-4">
                ...
                @if(!in_array($application->status, ['cancelled', 'paid', 'released', 'permit_generated']))
                    <form method="POST" action="{{ route('demolition-applications.cancel', $application) }}" class="inline" onsubmit="return confirm('Are you sure you want to cancel this application? This action cannot be undone.')" autocomplete="off">
                        @csrf
                ...
                @if($application->status === 'submitted' && !$application->assessments()->where('status', 'finalized')->exists())
                    ...
                    <div x-show="showRevertSubmitModal" x-cloak
                        ...
                        <div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6" @click.outside="showRevertSubmitModal = false">
                            ...
                            <form action="{{ route('demolition-applications.revertSubmission', $application) }}" method="POST" autocomplete="off">
                                @csrf
                                <div class="mb-4">
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Password <span class="text-red-500">*</span></label>
                ...
                @endif
                @endcan
                <a href="{{ route('demolition-applications.print', $application) }}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 transition">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
                <p class="text-xs text-gray-500">CTC Place Issued</p>
                <p class="text-sm text-gray-900 mt-0.5">{{ $application->applicant_ctc_place_issued ?? '---' }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        <h3 class="text-base font-semibold text-gray-900 border-b border-gray-200 pb-2 mb-4 flex items-center">
            <span class="inline-flex items-center justify-center w-7 h-7 bg-red-600 text-white text-xs font-bold rounded-full mr-2">{{ $sectionNum }}</span>Character of Occupancy
        </h3>
            ...
            <div class="space-y-2">
                @foreach($application->applicationOccupancyGroups as $occGroup)
                    <div class="flex items-start gap-2 text-sm">
                        <span class="inline-flex items-center justify-center w-6 h-6 bg-red-100 text-red-700 rounded-full text-xs font-medium shrink-0 mt-0.5">
                            {{ $loop->iteration }}
                        </span>
                        <div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-gray-900 mt-0.5">{{ $application->demolition_street ?? '---' }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-500">Barangay</p>
                <p class="text-sm text-gray-900 mt-0.5">{{ $application->demolitionBarangay?->name ?? '---' }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        <h3 class="text-base font-semibold text-gray-900 border-b border-gray-200 pb-2 mb-4 flex items-center">
            <span class="inline-flex items-center justify-center w-7 h-7 bg-red-600 text-white text-xs font-bold rounded-full mr-2">{{ $sectionNum }}</span>Scope of Work
        </h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <p class="text-xs text-gray-500">Scope</p>
                <p class="text-sm text-gray-900 mt-0.5">{{ $application->scope_of_work ? ucfirst($application->scope_of_work) : '---' }}</p>
            </div>
            <div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
                <p class="text-xs text-gray-500">Telephone</p>
                <p class="text-sm text-gray-900 mt-0.5">{{ $application->inspector_telephone ?? '---' }}</p>
            </div>
            <div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
                <p class="text-xs text-gray-500">CTC Place Issued</p>
                <p class="text-sm text-gray-900 mt-0.5">{{ $application->owner_ctc_place_issued ?? '---' }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        <h3 class="text-base font-semibold text-gray-900 border-b border-gray-200 pb-2 mb-4 flex items-center">
            <span class="inline-flex items-center justify-center w-7 h-7 bg-red-600 text-white text-xs font-bold rounded-full mr-2">{{ $sectionNum }}</span>Remarks
        </h3>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5">
        <h3 class="text-base font-semibold text-gray-900 border-b border-gray-200 pb-2 mb-4 flex items-center">
            <span class="inline-flex items-center justify-center w-7 h-7 bg-red-600 text-white text-xs font-bold rounded-full mr-2">{{ $sectionNum }}</span>Assessment Summary
        </h3>
        <div class="space-y-4">
            ...
            <div class="flex items-center justify-between pt-3 border-t border-gray-200">
                ...
                <span class="text-lg font-bold text-red-700">&#8369;{{ number_format($grandTotal, 2) }}</span>
            </div>
        </div>
    </div>
    @endif
