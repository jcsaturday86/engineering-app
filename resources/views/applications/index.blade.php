...
<div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex gap-2">
            <a href="{{ route('applications.create', ['type' => 'BP']) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition">
                <i class="fas fa-plus"></i> Building Permit
            </a>
            <a href="{{ route('applications.create', ['type' => 'OP']) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition">
                <i class="fas fa-plus"></i> Occupancy Permit
            </a>
        </div>
        @endcan
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4">
        <form method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div>
                ...
                <select name="permit_type" class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500">
                    <option value="">All</option>
                    @foreach($permitTypes as $pt)
                        <option value="{{ $pt->id }}" {{ request('permit_type') == $pt->id ? 'selected' : '' }}>{{ $pt->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            @if(request()->hasAny(['search', 'status', 'permit_type']))
