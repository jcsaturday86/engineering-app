...
@section('breadcrumbs')
    ...
    <i class="fas fa-chevron-right text-xs mx-2 text-gray-400"></i>
    <span class="text-gray-900 font-medium">Permits</span>
@endsection
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <h3 class="text-sm font-semibold text-gray-900">
                <i class="fas fa-file-alt text-gray-400 mr-2"></i>Generate Permits Report
            </h3>
        </div>
        <div class="p-6">
            <form method="POST" action="{{ route('reports.generate') }}" class="space-y-6">
                @csrf
                <input type="hidden" name="report_type" value="permits">
                ...
                <div>
                    ...
                    <select id="permit_type" name="permit_type"
                        ...
                        <option value="building" {{ old('permit_type') === 'building' ? 'selected' : '' }}>Building Permit</option>
                        <option value="occupancy" {{ old('permit_type') === 'occupancy' ? 'selected' : '' }}>Occupancy Permit</option>
                    </select>
                </div>
