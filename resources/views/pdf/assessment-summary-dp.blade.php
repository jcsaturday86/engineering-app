...
@php
    ...
    $location = collect([$application->demolition_street ?? '', $barangayName, ($settings['general.city'] ?? ''), ($settings['general.province'] ?? '')])
                ->filter()->join(', ');
    $printDate = now()->format('m/d/Y');
    $barcodeVal = $referenceNo ?? ('DP-' . $application->app_year . '-' . str_pad($application->app_month,2,'0',STR_PAD_LEFT) . '-' . str_pad($application->app_counter,5,'0',STR_PAD_LEFT));
@endphp
...
{{-- ═══════════════ TITLE ═══════════════ --}}
<div class="soc-title">DEMOLITION PERMIT ASSESSMENT</div>
<div class="soc-subtitle">SUMMARY OF COMPUTATION</div>

@php
$grandTotal = 0;

$demoItems = $itemsByCategory->get('DEMO_FEE', collect());
@endphp

{{-- ═══════════════ 1. DEMOLITION/MOVING FEES ═══════════════ --}}
@php
$demoSub = $demoItems->sum('amount') + $demoItems->sum('inspection_fee');
$grandTotal += $demoSub;
@endphp
<div class="section">
    <div class="sec-header">1. DEMOLITION/MOVING OF BUILDING/STRUCTURES FEES</div>
    <table class="ft" cellpadding="0" cellspacing="0">
        ...
        @forelse($demoItems as $idx => $item)
        @php $isLast = $idx === $demoItems->count() - 1; @endphp
        <tr>
            ...
            <td class="col-sub">@if($isLast && $demoSub > 0)P &nbsp;{{ number_format($demoSub, 2) }}@endif</td>
        </tr>
        @empty
