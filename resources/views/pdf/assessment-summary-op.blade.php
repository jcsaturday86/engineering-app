...
/* ── Title ── */
.soc-title { text-align: center; font-weight: bold; text-decoration: underline; font-size: 11px; margin: 8px 0 2px; }
.soc-subtitle { text-align: center; font-weight: bold; font-size: 10px; margin: 0 0 10px; }
...
@php
    ...
    $barcodeVal = ($application->permitType?->code ?? 'OP') . '-' . $application->app_year . '-' . str_pad($application->app_month,2,'0',STR_PAD_LEFT) . '-' . str_pad($application->app_counter,5,'0',STR_PAD_LEFT);
@endphp
...
{{-- ═══════════════ TITLE ═══════════════ --}}
<div class="soc-title">OCCUPANCY PERMIT ASSESSMENT</div>
<div class="soc-subtitle">SUMMARY OF COMPUTATION</div>

@php
$grandTotal = 0;

$occItems = $itemsByCategory->get('OCC', collect());
@endphp

{{-- ═══════════════ 1. OCCUPANCY FEES ═══════════════ --}}
@php
$occSub = $occItems->sum('amount') + $occItems->sum('inspection_fee');
$grandTotal += $occSub;
@endphp
<div class="section">
    <div class="sec-header">1. OCCUPANCY FEES</div>
    <table class="ft" cellpadding="0" cellspacing="0">
        ...
        @forelse($occItems as $idx => $item)
        @php $isLast = $idx === $occItems->count() - 1; @endphp
        <tr>
            ...
            <td class="col-sub">@if($isLast && $occSub > 0)P &nbsp;{{ number_format($occSub, 2) }}@endif</td>
        </tr>
        @empty
