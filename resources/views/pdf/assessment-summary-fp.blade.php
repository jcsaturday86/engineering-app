...
@php
    ...
    $location = collect([$application->construction_street ?? '', $barangayName, ($settings['general.city'] ?? ''), ($settings['general.province'] ?? '')])
                ->filter()->join(', ');
    $printDate = now()->format('m/d/Y');
    $barcodeVal = $referenceNo ?? ('FP-' . $application->app_year . '-' . str_pad($application->app_month,2,'0',STR_PAD_LEFT) . '-' . str_pad($application->app_counter,5,'0',STR_PAD_LEFT));
@endphp
    ...
    <tr>
        <td style="width:75%">
            <table cellpadding="0" cellspacing="0">
                ...
                <tr>
                    <td class="info-label" style="white-space:nowrap">Applicant:</td>
...
{{-- ═══════════════ TITLE ═══════════════ --}}
<div class="soc-title">FENCING PERMIT ASSESSMENT</div>
<div class="soc-subtitle">SUMMARY OF COMPUTATION</div>

@php
$grandTotal = 0;

$fencingItems = $itemsByCategory->get('FP_FEE', collect());
@endphp

{{-- ═══════════════ 1. FENCING PERMIT FEES ═══════════════ --}}
@php
$fencingSub = $fencingItems->sum('amount') + $fencingItems->sum('inspection_fee');
$grandTotal += $fencingSub;
@endphp
<div class="section">
    <div class="sec-header">1. FENCING PERMIT FEES</div>
    <table class="ft" cellpadding="0" cellspacing="0">
        ...
        @forelse($fencingItems as $idx => $item)
        @php $isLast = $idx === $fencingItems->count() - 1; @endphp
        <tr>
            ...
            <td class="col-sub">@if($isLast && $fencingSub > 0)P &nbsp;{{ number_format($fencingSub, 2) }}@endif</td>
        </tr>
        @empty
