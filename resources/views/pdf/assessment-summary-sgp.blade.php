...
@php
    ...
    $location = collect([$application->applicant_street ?? '', $barangayName, ($settings['general.city'] ?? ''), ($settings['general.province'] ?? '')])
                ->filter()->join(', ');
    $printDate = now()->format('m/d/Y');
    $barcodeVal = $referenceNo ?? ('SGP-' . $application->app_year . '-' . str_pad($application->app_month,2,'0',STR_PAD_LEFT) . '-' . str_pad($application->app_counter,5,'0',STR_PAD_LEFT));
@endphp
...
{{-- ═══════════════ TITLE ═══════════════ --}}
<div class="soc-title">SIGNAGE PERMIT ASSESSMENT</div>
<div class="soc-subtitle">SUMMARY OF COMPUTATION</div>

@php
$grandTotal = 0;

$signageItems = $itemsByCategory->get('SGP_FEE', collect());
@endphp

{{-- ═══════════════ 1. SIGNAGE PERMIT FEES ═══════════════ --}}
@php
$signageSub = $signageItems->sum('amount') + $signageItems->sum('inspection_fee');
$grandTotal += $signageSub;
@endphp
<div class="section">
    <div class="sec-header">1. SIGNAGE PERMIT FEES</div>
    <table class="ft" cellpadding="0" cellspacing="0">
        ...
        @forelse($signageItems as $idx => $item)
        @php $isLast = $idx === $signageItems->count() - 1; @endphp
        <tr>
            ...
            <td class="col-sub">@if($isLast && $signageSub > 0)P &nbsp;{{ number_format($signageSub, 2) }}@endif</td>
        </tr>
        @empty
