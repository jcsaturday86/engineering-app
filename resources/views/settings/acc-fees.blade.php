...
@section('title', 'Accessory Misc. Fees')
...
@section('breadcrumbs')
    ...
    <i class="fas fa-chevron-right text-xs mx-2 text-gray-400"></i>
    <span class="text-gray-900 font-medium">Accessory Misc. Fees</span>
@endsection

@section('content')
<div class="space-y-6">
    <div>
        <h2 class="text-xl font-bold text-gray-900">Accessory Miscellaneous Fees (ACC)</h2>
        <p class="text-sm text-gray-500 mt-1">
            Fee rates for miscellaneous accessory permit fees: line & grade, ground preparation, fencing, pavement, signs, repairs, and demolition.
        </p>
    </div>
    ...
    $sectionColors = [
        ...
        'slate'  => ['bg' => 'bg-slate-600',  'badge' => 'bg-slate-100 text-slate-700'],
                    ...
                    <table class="w-full text-xs">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr x-data="{ editing: false }" class="hover:bg-gray-50">
                                        ...
                                        <div class="flex items-center justify-between">
                                            <div class="grid grid-cols-8 gap-2 flex-1 text-gray-700">
                                                ...
                                                <span class="text-right {{ $schedule->percentage > 0 ? 'text-gray-700' : 'text-gray-300' }}">
                                                    {{ $schedule->percentage > 0 ? number_format($schedule->percentage * 100, 1).'%' : '—' }}
                                                </span>
                                            ...
                                            <div class="flex items-center gap-2 ml-3 shrink-0">
                                                @if($schedule->formula)
                                                <span class="text-xs bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded font-mono">{{ $schedule->formula }}</span>
                                                @endif
                                                <button @click="editing = true" class="text-blue-500 hover:text-blue-700" title="Edit">
                                                    <i class="fas fa-pencil-alt"></i>
                                                </button>
                                                <form action="{{ route('settings.acc-fees.schedule.destroy', $schedule) }}" method="POST"
                                ...
                                <template x-if="editing">
                                    <td colspan="9" class="px-3 py-2">
                                        <form action="{{ route('settings.acc-fees.schedule.update', $schedule) }}"
                            ...
                            <tr>
                                <td colspan="9" class="px-3 py-2 bg-gray-50">
                                    ...
                                    <div x-show="adding" x-cloak>
                                        <form action="{{ route('settings.acc-fees.schedule.store', $feeType) }}"
