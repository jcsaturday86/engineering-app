...
@section('breadcrumbs')
    ...
    <i class="fas fa-chevron-right text-xs mx-2 text-gray-400"></i>
    <span class="text-gray-900 font-medium">Demolition Fees</span>
@endsection

@section('content')
<div class="space-y-6">
    <div>
        <h2 class="text-xl font-bold text-gray-900">Demolition/Moving of Building/Structures Fees</h2>
        <p class="text-sm text-gray-500 mt-1">
            Fee rates for demolition and moving permit assessment, per sq. meter of area or dimensions involved.
        </p>
    </div>
    ...
    @php $clr = $sectionColors[$section['color']] ?? $sectionColors['red']; @endphp
                ...
                <div class="overflow-x-auto rounded-lg border border-gray-200">
                    <table class="w-full text-xs">
                        <thead class="bg-gray-50 border-b border-gray-200">
                            <tr>
                                <th class="text-right px-3 py-2 font-medium text-gray-500 w-32">Fixed Fee</th>
                                <th class="text-right px-3 py-2 font-medium text-gray-500 w-32">Fee/Unit</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr x-data="{ editing: false }" class="hover:bg-gray-50">
                                ...
                                <template x-if="!editing">
                                    <td colspan="3" class="px-3 py-2">
                                        <div class="flex items-center justify-between">
                                            <div class="grid grid-cols-2 gap-4 flex-1 text-gray-700">
                                            ...
                                            <div class="flex items-center gap-2 ml-3 shrink-0">
                                                ...
                                                <form action="{{ route('settings.demolition-fees.schedule.destroy', $schedule) }}" method="POST"
                                ...
                                <template x-if="editing">
                                    <td colspan="3" class="px-3 py-2">
                                        <form action="{{ route('settings.demolition-fees.schedule.update', $schedule) }}"
                                            ...
                                            <div>
                                                ...
                                                <input type="number" name="fixed_fee" value="{{ $schedule->fixed_fee }}" step="0.01" min="0"
                                                    class="w-32 px-2 py-1 border border-gray-300 rounded text-xs focus:ring-1 focus:ring-blue-500" placeholder="0.00">
                                            </div>
                                            <div>
                                                ...
                                                <input type="number" name="fee_per_unit" value="{{ $schedule->fee_per_unit }}" step="0.0001" min="0"
                                                    class="w-32 px-2 py-1 border border-gray-300 rounded text-xs focus:ring-1 focus:ring-blue-500" placeholder="0.0000">
                                            </div>
                            ...
                            <tr>
                                <td colspan="3" class="px-3 py-2 bg-gray-50">
                                    ...
                                    <div x-show="adding" x-cloak>
                                        <form action="{{ route('settings.demolition-fees.schedule.store', $feeType) }}"
                                            method="POST" class="flex items-end gap-2" autocomplete="off">
                                            @csrf
                                            <div>
                                                <label class="block text-xs text-gray-500 mb-0.5">Fixed Fee</label>
                                                <input type="number" name="fixed_fee" step="0.01" min="0"
                                                    class="w-32 px-2 py-1.5 border border-gray-300 rounded-lg text-xs focus:ring-1 focus:ring-blue-500" placeholder="0.00">
                                            </div>
                                            <div>
                                                <label class="block text-xs text-gray-500 mb-0.5">Fee/Unit</label>
                                                <input type="number" name="fee_per_unit" step="0.0001" min="0"
                                                    class="w-32 px-2 py-1.5 border border-gray-300 rounded-lg text-xs focus:ring-1 focus:ring-blue-500" placeholder="0.0000">
                                            </div>
