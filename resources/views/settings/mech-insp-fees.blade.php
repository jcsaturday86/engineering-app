...
@section('title', 'Mechanical Inspection Fees')
...
@section('breadcrumbs')
    ...
    <i class="fas fa-chevron-right text-xs mx-2 text-gray-400"></i>
    <span class="text-gray-900 font-medium">Mech Inspection Fees</span>
@endsection

@section('content')
<div class="space-y-6">
    <div>
        <h2 class="text-xl font-bold text-gray-900">Mechanical Permit Inspection Fees</h2>
        <p class="text-sm text-gray-500 mt-1">
            NBC annual inspection fee rates used in the mechanical permit assessment. Organized by NBC section (A–O).
        </p>
    </div>
    ...
    $sectionColors = [
        ...
        'yellow' => ['bg' => 'bg-yellow-500',  'badge' => 'bg-yellow-100 text-yellow-700'],
    ...
    <div x-data="{ open: {{ $loop->first ? 'true' : 'false' }} }" class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <button @click="open = !open" type="button"
            ...
            <div class="flex items-center gap-3">
                <span class="inline-flex items-center justify-center w-8 h-8 {{ $clr['bg'] }} text-white text-sm font-bold rounded-lg">
                    {{ $letter }}
                </span>
                <div>
                ...
                <div class="overflow-x-auto rounded-lg border border-gray-200">
                    <table class="w-full text-xs">
                        <thead class="bg-gray-50 border-b border-gray-200">
                            <tr>
                                <th class="text-left px-3 py-2 font-medium text-gray-500 w-24">Range From</th>
                                <th class="text-left px-3 py-2 font-medium text-gray-500 w-24">Range To</th>
                                ...
                                <th class="text-right px-3 py-2 font-medium text-gray-500 w-24">Excess ≥</th>
                                <th class="text-right px-3 py-2 font-medium text-gray-500 w-24">Excess Fee</th>
                                <th class="text-right px-3 py-2 font-medium text-gray-500 w-16">Every</th>
                                <th class="text-right px-3 py-2 font-medium text-gray-500 w-20">Action</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr x-data="{ editing: false }" class="hover:bg-gray-50">
                                ...
                                <template x-if="!editing">
                                    <td colspan="8" class="px-3 py-2">
                                        <div class="flex items-center justify-between">
                                            <div class="grid grid-cols-7 gap-2 flex-1 text-gray-700">
                                            ...
                                            <div class="flex items-center gap-2 ml-3 shrink-0">
                                                ...
                                                <form action="{{ route('settings.mech-insp-fees.schedule.destroy', $schedule) }}" method="POST"
                                ...
                                <template x-if="editing">
                                    <td colspan="8" class="px-3 py-2">
                                        <form action="{{ route('settings.mech-insp-fees.schedule.update', $schedule) }}"
                                            ...
                                            <input type="number" name="range_from" value="{{ $schedule->range_from }}" step="0.01" min="0"
                                                class="w-24 px-2 py-1 border border-gray-300 rounded text-xs focus:ring-1 focus:ring-blue-500" placeholder="From">
                                            <input type="number" name="range_to" value="{{ $schedule->range_to }}" step="0.01" min="0"
                                                class="w-24 px-2 py-1 border border-gray-300 rounded text-xs focus:ring-1 focus:ring-blue-500" placeholder="To">
                                            ...
                                            <input type="number" name="excess_threshold" value="{{ $schedule->excess_threshold }}" step="0.01" min="0"
                                                class="w-24 px-2 py-1 border border-gray-300 rounded text-xs focus:ring-1 focus:ring-blue-500" placeholder="Excess ≥">
                                            ...
                                            <input type="number" name="excess_every" value="{{ $schedule->excess_every }}" step="1" min="1"
                                                class="w-16 px-2 py-1 border border-gray-300 rounded text-xs focus:ring-1 focus:ring-blue-500" placeholder="Every">
                            ...
                            <tr>
                                <td colspan="8" class="px-3 py-2 bg-gray-50">
                                    ...
                                    <div x-show="adding" x-cloak>
                                        <form action="{{ route('settings.mech-insp-fees.schedule.store', $feeType) }}"
                                            ...
                                            <div>
                                                ...
                                                <input type="number" name="excess_every" step="1" min="1" value="1"
                                                    class="w-16 px-2 py-1.5 border border-gray-300 rounded-lg text-xs focus:ring-1 focus:ring-blue-500" placeholder="1">
                                            </div>
